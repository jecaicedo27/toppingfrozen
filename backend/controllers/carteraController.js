  ...
  try {
    ...
    const sqlBodega = `
      SELECT
        ...
        o.customer_address COLLATE utf8mb4_unicode_ci AS customer_address,
        o.total_amount,
        COALESCE(cr.payment_method, o.payment_method) COLLATE utf8mb4_unicode_ci AS payment_method,
        o.shipping_payment_method COLLATE utf8mb4_unicode_ci AS shipping_payment_method,
...
const getHandovers = async (req, res) => {
  try {
    ...
    const whereBodega = ['cr.status = "collected"', 'cr.payment_method = "efectivo"'];
    const paramsBodega = [];
    if (from) {
...
const getBodegaHandoverDetails = async (req, res) => {
  try {
    ...
    const items = await query(
      ...
      LEFT JOIN users ua ON ua.id = cr.accepted_by
      LEFT JOIN users ur ON ur.id = cr.registered_by
      WHERE DATE(cr.accepted_at) = ? AND cr.status = 'collected' AND cr.payment_method = 'efectivo'${originFilter}
