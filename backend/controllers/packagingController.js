...
const startPackaging = async (req, res) => {
  try {
    ...
    const sqlQuery = `
      UPDATE orders 
      SET status = 'en_empaque', updated_at = NOW()
      WHERE id = ?
    `;
...
const completePackaging = async (req, res) => {
  try {
    ...
    // Y resetear flags de modificación (snapshot nuevo)
    const currentItems = await query('SELECT * FROM order_items WHERE order_id = ?', [orderId]);
    const snapshot = JSON.stringify(currentItems);
    ...
    const sqlQuery = `
      ...
      SET status = 'listo_para_entrega', 
          is_modified_after_packing = 0, 
          packing_snapshot = ?,
          updated_at = NOW()
      WHERE id = ?
    `;

    console.log(`🔧 Ejecutando UPDATE para pedido ${orderId}: status = 'listo_para_entrega', clearing flags`);
    const updateResult = await query(sqlQuery, [snapshot, orderId]);
...
const finalizarEmpaque = async (req, res) => {
  try {
    ...
    let newStatus = approvalStatus === 'approved' ? 'listo_para_entrega' : 'en_empaque';
...
const uploadPackagingEvidence = async (req, res) => {
  try {
    ...
    for (const f of req.files) {
      ...
      const apiPath = `/api/public/evidence/${f.filename}/content`;
      await query(
      ...
      saved.push({ filename: f.filename, url: apiPath, size: f.size, type: f.mimetype });
    }
...
const listPackagingEvidence = async (req, res) => {
  try {
    ...
    const rows = await query(
      `SELECT id, photo_filename, photo_path, description, taken_at, created_at, photo_size AS size, photo_type AS type
    ...
    // Construct full URL for frontend usage
    const files = rows.map(r => ({
      ...r,
      url: `/api/public/evidence/${r.photo_filename}/content`
    }));
