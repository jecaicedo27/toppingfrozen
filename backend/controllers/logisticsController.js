  ...
  try {
    ...
    const readyOrders = await query(
      `SELECT 
        ...
        (SELECT GROUP_CONCAT(DISTINCT payment_method SEPARATOR ', ') FROM cash_register WHERE order_id = o.id AND status IN ('pending', 'collected', 'accepted')) as registered_payment_methods,
...
const receivePickupPayment = async (req, res) => {
  try {
    ...
    console.log('[DEBUG receivePickupPayment] Files:', req.files?.length || 0);
    ...
    if (!isCartera && !isAdmin) {
      return res.status(403).json({ success: false, message: 'Acceso denegado: se requiere rol Cartera o Admin.' });
    }

    const { orderId, payments: paymentsJson, notes } = req.body || {};
    ...
    const rows = await query(
      'SELECT id, order_number, status, delivery_method, total_amount FROM orders WHERE id = ?',
      [orderId]
    );
    ...
    const order = rows[0];
    const deliveryMethod = (order.delivery_method || 'recoge_bodega').toLowerCase();
    ...
    try {
      ...
      return res.status(400).json({ success: false, message: 'Formato de pagos inválido' });
    }

    if (!Array.isArray(payments) || payments.length === 0) {
      return res.status(400).json({ success: false, message: 'Se requiere al menos un pago' });
    }

    // Iniciar transacción para asegurar integridad
    await transaction(async (tx) => {
      let photoOrder = 0; // Para rastrear qué foto corresponde a qué pago basado en photoIndices del frontend

      for (const p of payments) {
        const method = (p.method || 'efectivo').toLowerCase();
        const amt = Number(p.amount) || 0;
        const evidenceStr = (p.photoIndices && p.photoIndices.length > 0)
          ? `Múltiples evidencias (${p.photoIndices.length})`
          : 'Sin evidencia fotográfica';

        // 1. Insertar en cash_register (solo si el monto es mayor a cero)
        ...
        if (amt > 0) {
          const crResult = await tx.query(
            ...
            [
              ...
              deliveryMethod,
              userId || null,
              `${evidenceStr}${notes ? ' - ' + notes : ''}`
            ]
          );
          cashRegisterId = crResult.insertId;
        }

        // 2. Vincular fotos si existen (solo si se registró un movimiento en caja)
        if (cashRegisterId && req.files && req.files.length > 0 && p.photoIndices) {
          for (let i = 0; i < p.photoIndices.length; i++) {
            const file = req.files[photoOrder];
            if (file) {
              const file_path = `/uploads/delivery_evidence/${file.filename}`;
              await tx.query(
                `INSERT INTO payment_evidences (order_id, cash_register_id, file_path, uploaded_at, uploaded_by)
                 VALUES (?, ?, ?, NOW(), ?)`,
                [orderId, cashRegisterId, file_path, userId]
              );
              photoOrder++;
            }
          }
      ...
      // 3. Avanzar estado del pedido
      if (order.status === 'revision_cartera') {
        await tx.query(
          `UPDATE orders SET status = 'en_logistica', updated_at = NOW() WHERE id = ?`,
          [orderId]
        );
        console.log(`✅ Pagos registrados - Pedido ${order.order_number} movido a en_logistica`);
        emitStatusChange(order.id, order.order_number, order.status, 'en_logistica');
      } else {
        await tx.query('UPDATE orders SET updated_at = NOW() WHERE id = ?', [orderId]);
      }
    });
...
const markPickupDelivered = async (req, res) => {
  try {
    ...
    // - Si el método de pago del pedido es efectivo/contado/contraentrega o es un pago mixto (transferencia + efectivo),
    //   exigir que el saldo pendiente sea 0.
    if (isPickupMethod) {
      const cashLikeStrong = ['efectivo', 'contado', 'contraentrega', 'cash'].some(k => pmNorm.includes(k));
      const isTransfer = pmNorm.includes('transferencia');

      if (cashLikeStrong || isTransfer) {
        // Consultar el total registrado en caja (collected) y el total pagado (paid_amount)
        const paymentStats = await query(
          `SELECT 
            o.total_amount,
            o.paid_amount,
            (SELECT COALESCE(SUM(amount), 0) FROM cash_register WHERE order_id = o.id AND status = "collected") as total_cash_collected
           FROM orders o 
           WHERE o.id = ?`,
          [orderId]
        );
        ...
        if (paymentStats.length > 0) {
          const { total_amount, paid_amount, total_cash_collected } = paymentStats[0];
          const totalPaid = Number(paid_amount || 0) + Number(total_cash_collected || 0);
          const balance = Number(total_amount || 0) - totalPaid;

          if (balance > 1) { // Tolerancia de 1 peso por decimales
            return res.status(400).json({
              success: false,
              message: `El pedido tiene un saldo pendiente de $${balance.toLocaleString()}. Registre el pago completo antes de ENTREGAR.`
            });
          }
        }
