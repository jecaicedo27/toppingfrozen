...
const InventoryManagementPage = () => {
    ...
    const extractPresentation = (productName, productCode) => {
        ...
        let match = normalized.match(/(?:\bX\b\s*)?(\d+(?:\.\d+)?)\s*(ML|GRS|GR?|KG|L|G|OZ|UND|UNIDADES)\b/i);
        if (!match) {
            match = normalized.match(/(\d+(?:\.\d+)?)\s*(ML|GRS|GR?|KG|L|G|OZ|UND|UNIDADES)\b/i);
        }
        ...
        if (match) {
            ...
            if (unitToken === 'ML' || unitToken === 'L' || normalized.includes('ML') || normalized.includes('SIROPE') || normalized.includes('LIQUIDO')) {
                ...
                unit = 'OZ';
            } else if (unitToken === 'UND' || unitToken === 'UNIDADES') {
                unit = 'UND';
            }
            ...
            // Normalización de GRS a G
            if (unitToken === 'GRS') unit = 'G';

            if (value === '250') return unit === 'ML' ? '250ML' : (unit === 'UND' ? '250UND' : '250G');
            ...
            if (value === '500') return unit === 'ML' ? '500ML' : (unit === 'UND' ? '500UND' : '500G');
            if (value === '1000') return unit === 'ML' ? '1000ML' : (unit === 'UND' ? '1000UND' : '1000G');
            ...
            if (value === '3000') return unit === 'ML' ? '3000ML' : (unit === 'UND' ? '3000UND' : '3000G'); // Agregado para Gomas 3000g
            if (value === '3400') return unit === 'ML' ? '3400ML' : '3400G';

            // Retorno genérico si no cayó en reglas específicas
            return `${value}${unit}`;
        }
    ...
    const organizeProductsForInventory = (productsList) => {
        ...
        productsList.forEach(product => {
            ...
            // 0. Filtros globales de exclusión (Servicios, Domicilios, etc)
            if (product.internal_code === 'DO' || product.name === 'DOMICILIOS' || product.type === 'Service') {
                return;
            }
            ...
            // Override específico solicitado por el usuario
            if (product.internal_code === 'MEP26' || product.internal_code === 'MES26') {
                subgroup = 'HELADO PREMIUM';
            }
            if (product.name && product.name.toUpperCase().includes('FINI')) {
                subgroup = 'TOPPING IMPORTADOS';
            }

            // Regla dinámica para TOPPINGS basada en Código Interno (TO + X)
            if (product.internal_code && product.internal_code.startsWith('TO')) {
                group = 'TOPPING';
                if (product.internal_code.startsWith('TOI')) subgroup = 'TOPPING IMPORTADOS';
                else if (product.internal_code.startsWith('TOD')) subgroup = 'TOPPINGS DULCE';
                else if (product.internal_code.startsWith('TOC')) subgroup = 'TOPPING CHOCOLATE';
            }

            if (product.internal_code === 'TOD07') {
                subgroup = 'TOPPINGS DULCE';
            }
            ...
            let flavor = extractFlavor(product.name, subgroup);
    ...
    const computeOrderedFlavorsWithDividers = (categoryData, category) => {
        ...
        // Calcular conteo de presentaciones por sabor
        const flavorCounts = {};
        flavorsSet.forEach(fl => {
            let count = 0;
            Object.values(categoryData).forEach(pres => {
                if (pres[fl]) count++;
            });
            flavorCounts[fl] = count;
        });

        Object.keys(groups).forEach(k => {
            groups[k].sort((a, b) => {
                // Mayor cantidad de presentaciones primero (descendente)
                const diff = flavorCounts[b] - flavorCounts[a];
                if (diff !== 0) return diff;
                // Si tienen la misma cantidad, alfabético
