  ...
  return (
    <div className="p-6">
        ...
        <div className="flex items-center space-x-2">
          {user?.role === 'admin' && (
            <button
              ...
              className="btn btn-primary flex items-center bg-blue-600 hover:bg-blue-700 text-white px-3 py-2 rounded-lg text-sm mr-2"
            >
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        {/* Actas / Cierres (Agrupados por fecha) */}
        <div className="card">
          <div className="card-header">
            <h2 className="text-lg font-semibold text-gray-900 flex items-center">
          ...
          <div className="card-content p-0">
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {groupedHandovers.map(([date, dateItems]) => (
                      ...
                      <tr className="bg-indigo-50/50">
                        <td colSpan={6} className="px-4 py-2">
                          <span className="text-xs font-bold text-indigo-800 flex items-center">
                            ...
                            {new Date(date).toLocaleDateString('es-CO', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric', timeZone: 'UTC' })}
                          </span>
                        </td>
                      </tr>
                      {dateItems.map((h) => (
                            ...
                            <td className="px-4 py-2">
                              <div className="text-sm font-medium text-gray-900">
                                {String(h.source || '').startsWith('bodega')
                                  ? `Cierre Bodega - ${h.source.includes('logistica') ? 'Logística' : 'Cartera'}`
                                  : `Cierre #${h.id} - ${h.messenger_name || 'Desconocido'}`
                                }
                              </div>
                              <div className="text-xs text-gray-500">
                                {h.closing_time ? new Date(h.closing_time || h.created_at).toLocaleTimeString('es-CO', { hour: '2-digit', minute: '2-digit' }) : (String(h.source || '').startsWith('bodega') ? 'Consolidado' : h.closing_date)}
