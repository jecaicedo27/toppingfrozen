...
const PackagingPage = () => {
  ...
  // Compute deleted items from snapshot
  const deletedItems = React.useMemo(() => {
    if (!currentOrder?.is_modified_after_packing || !currentOrder?.packing_snapshot) return [];
    let snapshot = currentOrder.packing_snapshot;
    ...
    return snapshot.filter(s => {
      // Return true if NOT found in checklist
      return !checklist.some(c => (c.product_code && c.product_code === s.product_code) || (c.item_name === s.name));
    });
  ...
  useEffect(() => {
    ...
    socket.on('connect', () => {
      try {
        ...
        socket.emit('join-packaging-updates');
      } catch { }
    });
    ...
    const handlePackagingProgress = (payload) => {
      try {
        // payload: { orderId, total_items, verified_items, pending_items, progress_pct }
        setPendingOrders(prev => prev.map(order => {
          if (order.id === payload.orderId) {
            return {
              ...
              // progress_pct también viene, pero lo calculamos en render si queremos
            };
          }
          return order;
        }));
      } catch (e) {
        console.error('Error actualizando progreso socket:', e);
      }
    };
          ...
          {groupedPending.filter(g => (g.items || []).length > 0).map(g => (
            <div key={g.key} className="mb-6 last:mb-0">
              ...
              <div className="md:hidden p-4 space-y-3">
                {g.items.map((order) => (
                  <div key={order.id} className={`rounded-lg border shadow-sm p-3 ${order.packaging_status === 'requires_review' ? 'ring-2 ring-yellow-400 ring-offset-1 bg-yellow-50' : 'bg-white'}`}>
                    <div className="flex items-start justify-between">
                      <div className="pr-3 min-w-0">
                        ...
                        {(() => {
                          ...
                          if (order.packaging_status === 'requires_review') {
                            return (
                              <div className="mt-1 text-xs inline-flex items-center px-2 py-0.5 rounded-full bg-yellow-100 text-yellow-800 border border-yellow-200 animate-pulse">
                                <Icons.AlertTriangle className="w-3 h-3 mr-1" />
                                Cambios SIIGO
                              </div>
                            );
              ...
              <div className="hidden md:block overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {g.items.map((order) => (
                      <tr key={order.id} className={order.packaging_status === 'requires_review' ? 'ring-2 ring-yellow-400 ring-inset bg-yellow-50' : 'bg-white'}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          {(() => {
                            ...
                            const verified = order.verified_items || 0;
                            const pct = total > 0 ? Math.round((verified / total) * 100) : 0;
                            return (
                              <div className="flex items-center">
                                <span className="text-xs font-bold text-gray-700 w-8 text-right mr-2">{pct}%</span>
                                <div className="w-24 bg-gray-200 rounded-full h-2.5">
                                  <div
                                    className={`h-2.5 rounded-full transition-all duration-500 ${pct === 100 ? 'bg-green-500' : pct > 0 ? 'bg-yellow-400' : 'bg-gray-300'}`}
                                    style={{ width: `${pct}%` }}
                                  ></div>
                                </div>
                              </div>
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          {currentOrder.packaging_status === 'requires_review' && (
            <div className="mb-4 p-3 border border-yellow-300 bg-yellow-50 rounded flex items-start">
              ...
              <div>
                <p className="text-sm text-yellow-900 font-medium">Pedido actualizado desde SIIGO</p>
                <p className="text-xs text-yellow-800">
                  Se detectaron modificaciones (productos añadidos, eliminados o cambio de cantidades). Por favor verifique.
                </p>
                <div className="mt-2">
                  <button
                    onClick={async () => {
        ...
        validationMode === 'manual' && (
          <>
            ...
            <div className="space-y-1">
              {checklist.map((item) => {
                // Logic to detect changes from Siigo Modify
                let diffBadge = null;
      ...
      {/* Modo Código de Barras */}
      {
        validationMode === 'barcode' && (
          <>
            ...
            <div id="scannerBar" className="fixed right-4 left-auto z-50 w-[calc(100%-2rem)] md:w-auto md:min-w-[400px] max-w-2xl" style={{ top: 'max(env(safe-area-inset-top, 8px), 8px)' }}>
