...
const MultiplePaymentEvidenceUpload = ({ orderId, onUploadComplete, onEvidencesChange, onFilesSelected, readOnly = false, showExistingEvidences = true }) => {
    ...
    const processFiles = (files) => {
        ...
        for (const file of files) {
            ...
            if (file.size > 50 * 1024 * 1024) {
                toast.error(`${file.name}: El archivo no puede ser mayor a 50MB`);
                continue;
            }
    ...
    return (
        <div className="space-y-4">
            {/* Sección de subida - Hidden if readOnly */}
            {!readOnly && (
                        ...
                        <div className="mt-4">
                            <label htmlFor="file-upload" className="cursor-pointer">
                                ...
                                <span className="mt-1 block text-xs text-gray-500">
                                    Imágenes o PDFs hasta 50MB (máximo 5 archivos)
                                </span>
                                <input
                                    id="file-upload"
                                    type="file"
            ...
            ) : (showExistingEvidences && existingEvidences.length > 0) ? (
                <div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                        {existingEvidences.map((evidence) => (
                            ...
                            >
                                <img
                                    src={`/api/${evidence.file_path}`}
                                    alt="Comprobante"
                                    className="w-full h-32 object-cover cursor-pointer"
                                    onClick={() => setPreviewImage(`/api/${evidence.file_path}`)}
                                />
