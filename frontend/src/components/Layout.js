...
const Layout = () => {
  ...
  const [businessControlsOpen, setBusinessControlsOpen] = useState(false);
  ...
  const navigation = [
    ...
    // Configuración submenu group
    {
      ...
      children: [
        ...
        {
          name: 'Config Bancolombia',
          href: '/bank-config',
          icon: Building,
          current: location.pathname === '/bank-config',
        },
        {
          name: 'Automatización',
          href: '/automation',
    ...
    },
    {
      ...
      icon: Coins,
      current: location.pathname === '/cashier-collections',
      requiresPermission: ['admin', 'cartera'],
    },
    // Controles Empresariales group
    {
      name: 'Controles Empresariales',
      icon: TrendingUp,
      requiresPermission: ['admin', 'cartera', 'facturador'],
      isGroup: true,
      children: [
        {
          ...
          icon: Calculator,
          current: location.pathname === '/financial-closure',
        },
        {
          ...
          <div className="flex-1 h-0 pt-5 pb-4 overflow-y-auto">
            ...
            <nav className="mt-5 px-2 space-y-1">
              {filteredNavigation.map((item) => {
                ...
                if (item.isGroup) {
                  const isConfigGroup = item.name === 'Configuración';
                  const isBusinessGroup = item.name === 'Controles Empresariales';
                  const isOpen = isConfigGroup ? configMenuOpen : (isBusinessGroup ? businessControlsOpen : false);
                  const toggleFunc = isConfigGroup ? setConfigMenuOpen : (isBusinessGroup ? setBusinessControlsOpen : null);

                  return (
                    <div key={item.name}>
                      <button
                        onClick={() => toggleFunc && toggleFunc(!isOpen)}
                        className="group w-full flex items-center px-2 py-2 text-base font-medium rounded-md text-gray-600 hover:bg-gray-50 hover:text-gray-900"
                      >
                          ...
                          {item.children.map((child) => {
                            ...
                            return (
                              <Link
                                key={child.name}
                                to={child.href}
                                className={`group flex items-center px-2 py-2 text-sm font-medium rounded-md ${child.current
